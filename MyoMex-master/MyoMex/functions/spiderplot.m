...
%
%       !!! input this to command window !!!
%           maxax = [1;1;1;1;1;1;1;1];
%           minax = [0;0;0;0;0;0;0;0];
%           h = spiderplot(x,maxax,minax,'b')
%           set(h.axis, 'LineWidth',2);
%           set(h.plots, 'LineWidth',1); 
%
%       where x is a [8x1] matrix with values normalized between 0 and 1
%        
% 
%  Usage:
...
%
%    Martti Kesniemi <martti.kesaniemi@mathworks.com>
...
if (nargin < 2)||(isempty(axisMax))
    ...
    axisMax = ceil(axisMax ./ 10.^floor(log10(abs(axisMax)))) .*...
        10.^floor(log10(abs(axisMax)));
else
    axisMax = axisMax(:);
...
if (nargin < 3)||(isempty(axisMin))
    if min(X(:)) < 0
        ...
        axisMin = floor(axisMin ./ 10.^floor(log10(abs(axisMin)))) .*...
            10.^floor(log10(abs(axisMin)));
    else
