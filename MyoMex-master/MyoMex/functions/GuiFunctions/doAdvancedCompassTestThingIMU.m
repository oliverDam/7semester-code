...
function doAdvancedCompassTestThingIMU(handles1, m1,sensX,sensY, handles2)
        ...
        while allPoint ~= 32
            ...
            if ~isempty(timeEMG)
                ...
                if lastSample >= windowSize && buffer1 >= windowSize
                    ...
                    %Some accelerometer stuff as well:
                    accelMatrix = m1.accel_log(iiIMU,:);                    
                    
                    %This is also ok featz cause we so streetz:
                    featz = featureExtractionLiveMAV(toBeFiltered);
                    getRegressValue = [getRegressValue;getRegressionValueIMU(featz,accelMatrix(end,:),mahExtensionRegrizzle, ...
    ...
    testResult = timeDif(1:2:end)*transpose(gotIt)/sum(gotIt);
