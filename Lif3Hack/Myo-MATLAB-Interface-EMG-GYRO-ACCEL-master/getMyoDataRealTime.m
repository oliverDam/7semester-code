...
% Mit bud vil være at C++ programmet gemmer alt i filerne, og dette program
% så henter resultaterne ind fra filerne. Lettere forvirrende, men det
% virker tydeligvis.
...
lineLengthACCEL = 47;
numChannels = 8;
emgSampMax = 4000; %Antal EMG samples der gemmes i matricen
gyroAccelSampMax = emgSampMax/4; %Antal EMG og Accel samples maks
windowLength = 40; 
curPositionEMG = 0;
curPositionGYRO = 0;
curPositionACCEL = 0;
...
while get(gcf,'currentchar')==']' % While no button has been pressed
    ...
    while ischar(fileDataRaw) % Extract new data from file
        ...
        if curSampleEMG >= windowLength % MAV feature extraction - han har implementeret et filter
    ...
    if curSampleEMG > emgSampMax % Clear arrays when large
        ...
        lastSampleGYRO = 1;
        lastSampleACCEL = 1;
        emgData = NaN([emgSampMax numChannels]); % Ikke helt sikker på hvordan, men her gemmes EMG-matricen
        mavData = NaN([(emgSampMax - windowLength) numChannels]); %resultater fra moving average value beregning
        gyroData = NaN([gyroAccelSampMax 3]); % og gyro-matrix
        accelData = NaN([gyroAccelSampMax 3]); % og accel-matrix
